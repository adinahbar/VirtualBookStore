package com.adinaandsari.virtualbookstore;

/**
 * Created by adina_000 on 20-Dec-15.
 */
public class ConstValue {
    final static String CUSTOMER_KEY = "com.adinaandsari.virtualbookstore.customer";
    final static String MANAGER_KEY = "com.adinaandsari.virtualbookstore.manger";
    final static String SUPPLIER_KEY = "com.adinaandsari.virtualbookstore.supplier";
    final static String PERSON_KEY = "com.adinaandsari.virtualbookstore.person";
    final static String BOOK_KEY = "com.adinaandsari.virtualbookstore.book";
    }
